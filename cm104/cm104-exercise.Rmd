...
to the left or `~` is the variable, and the RHS is the function we defined. 
[class note](http://stat545.com/Classroom/notes/cm104.nb.html)
...
```
```{r}
map2(a, b, ~ .x * .y)   # RHS of ~ is the function 
```
```{r}
map2(a, b, `*`)  # `*` itself is the function
# we can do the multiplication like the following
`*`(5,7)
```
```{r}
map2_dbl(a, b, `*`)
...
b <- c(4,5,6)
c <- c(7,8,9)
pmap(list(a, b, c), function(x, y, z) x*y*z)  # put all the variables we want to iterate over in a LIST. 
```
```{r}
pmap(list(a, b, c), ~ ..1 * ..2 * ..3)  # use ..1 means the first variable  (so when there is more than 2 variables, it is convention to name it ..1  ..2 etc)
```
```{r}
...
map2_chr(name, commute, comm_fun)  # variables need to be the equal length for the function to work .
```

2. By defining a function "on the fly" within the `map2()` function.

```{r}
map2_chr(name, commute, function(t, s) str_c(s, " takes ", t, " minutes to get to work."))
```
...
got_alias <- got_chars %>%{
  tibble(name = map_chr(.,"name"),
         aliases = map(.,"aliases"))}  # take everything from got_chars and subseting with the "name"
...
# setNames---return the vector of LHS with the name of RHS
```
...
```{r}
icecream %>% 
    unnest(flav)  # unnest the untidy dataframe by its flavours. 
```
...
```{r}
icecream %>% 
  filter(map_lgl(flav, ~ any(.x == "strawberry")))  # list the name contain strawberry flavours.
```


## Nesting/Unnesting: Your Turn

`unnest()` the `got_alias` tibble. Hint: there should be a hiccup. Check out the `str()`ucture of `got_alias` -- are all elements of the list column vectors? Would using `tidyr::drop_na()` be a good idea here?  ---- drop all the enrty with NA
