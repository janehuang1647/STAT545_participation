...
- hold multiple entries of the __same type__.
...
- subset with `[`, `[[`, or `$`. 
- `[` means i would like to have the list, `[[` means i would like to have the content of the list.
...
```{r}
(1:10) * (1:2)  # cycling (LHS should be equal length to RHS)
```
...
for (i in 1: length(wesanderson))
    x[i] <- wesanderson[[i]][1]   # subsetting to get the content of the vector
...
# `list.len`tell us how long the list we wanna view
# `max.level` only show us thet top # level. such as we only want to see the section but not the subsubsection. 
```{r}
...
# useful for nested structure data
```{r}
View(got_chars)
...
```{r}
x <- 1:10
sqrt(x) # automactically vectorize
map(x, sqrt)  # map ALWAYS output a LIST. 
```
...
```{r}
map_dbl(x, sqrt)  # with this we are constructing a vector-typed double
map_chr(x, sqrt)   # with this we are constructing a vector-typed characters
```

Does your function have other arguments? You can specify them afterwards in the ellipses (`...`).

```{r}
map_chr(x, str_c, "potato.", sep="-")  # str_c Join multiple strings into a single string
```
...
## method 2: head function
map_chr(wesanderson,head, n=1)  # only subseting the first element (by default, n = 6)
# map_dbl(wesanderson,head, n=1) can not tranfer to a double

## method 3
map_chr(wesanderson,`[`,1 )  # `[`-- subseting x to the first element    `[` means x[1]
```
...
```{r}
map_chr(wesanderson, 1) %>% unname()   # this takes out the name of the vector
```
...
```{r}
got_chars %>%   # subset according to "titles" elements
  map("titles")

got_chars %>% 
  map(9)  # the 9th element is titles so it is the same as above.
...
map(got_chars, `[`, desired_info) %>% 
  head() # only getting the first vector
## or
map_dfr(got_chars,`[`, desired_info)  # list everything in the data frame
```
...
got_chars %>% {
    ...
    tibble(
        name = map_chr(., "name"),   # . means whatever element to the left of %>% 
        born = map_chr(., "born")
    )
}
...
# this is giving us a three column lists  it will make got_chars as a own column but with a curly braces, this column can be avoided. (check  ?`}` for more information)

# in the curly bracket, it only return the execution of the last line. 
