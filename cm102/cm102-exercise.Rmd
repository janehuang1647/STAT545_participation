...
__ string is pretty much interchangable with character__
...
```{r}
# separate the string by the space
str_split(fox, pattern = " ")[[1]] %>% length()  # return the length of the string (8 words)
...
fox %>% 
  str_split("") %>% 
  `[[`(1) %>% # means subseting the first part of the first section
  str_to_lower() %>% 
table()
...
fruit %>% head
fruit %>% 
    str_pad(width=7, side="right", pad="$") %>% # if we want to add more character, add new character to the right (to make the string with a string length of 7)
    head()
```
...
```{r}
str_length(fruit) # to see the length of each entry in the fruit
length(fruit) # return the length of the string fruit instead of each entries inside the string
```


`str_c()` for __concatenating strings__. Check the docs for an excellent explanation using a matrix.

```{r}
str_c(words[1:4], words[5:8], sep=" & ") # concatenate pairwise between two groups
str_c(words[3:4], words[5:8], sep=" & ")
str_c(words[3:4], words[5:8], sep=" & ", collapse=", ") # collapes is combining the results again separated by a comma. (collapse augument collaspe all the vectors together to make a single vector.)
```
...
```{r}
# create something like cm103/README.md
...
cat(contents[1])  # to see whether it is work (print out the correct format)
```
...
# Italy is not shown here since the I is capitalised and the function is case sensitive
```

__Here, `.` stands for "any single character".__
...
What if I wanted to literally search for countries with __a period__ in the name? Escape with `\`, although R requires a double escape.

```{r}
str_view_all_match(countries, pattern = "\\.")  # TO INDICATE THAT we actually want a period in the search not just indicating any characters.
```
...
```{r}
c("bear", "beer", "bar") %>% 
    str_view_all_match(pattern = "be(e|a)r") # search all word in this form
```
...
```{r}
str_view_all_match(countries, "^[^aeiouAEIOU]*$") ## staring with a non-vowel * means 0 or more characters before
```
...
```{r}
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\1")  # want whatever results in group one
str_view_all(c("abad", "abbd"), pattern="(a)(b)\\2") # a followed by b and whatever in group 2 of the third element
```

Note that the parentheses are first resolved, THEN referred to. NOT re-executed.

```{r}
str_view_all(c("bananas"), "(.)(.)\\1\\2.*\\1\\2")    # anythings follow by anything, then repeat the first group and then the second group and anything in between
```
...
str_view_all(c("bananas", "Who can? Bananas can."), "(.)(.)\\1\\2.*\\1\\2") 
                                                    # a  n   a  n .. a  n
#  .* means any number of anything

# . means it can takes anything and so for the banana examples,  first group we can fit in a and second group we can fit in n and \\1 followed by group 1 will be a and \\2 followed by group 2 will be n.
```
