...
# define the function  the default is to execute the last line of the curly bracket and return the result
# if we put whatever things after return, it will not effect the function (in this case, it will still return y.)
square(10)
```
...
?min
?max
pmax(c(1,3,4),c(2,1,1))  # this function compare pairwise entry by entry
...
```{r}
max_minus_min(1:10)
max_minus_min(runif(100))  # runif uniform distribution
```

Let's formalize this testing with the `testthat` package. `expect_*()` functions:

```{r}
expect_equal(0.1 + 0.2, 0.3)  # if the test passed, it will not return anything
# expect_identical(0.1 + 0.2, 0.3)  # stronger function than expect_equal (it is not identical due to some floating point error)

# so it either gives nothings or error.
```

Add another check to the following unit test, based on the uniform random numbers:

```{r}
# the name of the test (this will show up while the test failed)
test_that("Simple cases work", {
    expect_equal(max_minus_min(1:10), 9)
    expect_lt(max_minus_min(runif(100)),1) # random number generated between 0 and 1
    expect_less_than(max_minus_min(runif(100)),1)
    # expect_lt less than the compared value. 
})
```
...
# not working in the complete dataframe
#max_minus_min(gapminder) 
#max_minus_min(gapminder$country) not working for the whole vector
```
...
```{r}
stopifnot(FALSE) # take in a logical variable
...
# mmm1("hello") # string is not a numeric, therefore there is error
```
...
# mmm2(gapminder[c('lifeExp', 'gdpPercap', 'pop')]) # character is an interger!
...
0.9 quantiles: it is a number that could be over with 10 % chance. (90% of number will fall below)

```{r}
# example: quantile(rnorm(100),c(0.1,0.9))
qd <- function(x, probs) {
    stopifnot(is.numeric(x))
    if (length(x) == 0) {
...
```{r}
x <- runif(100)
qd(x, c(0.25, 0.75)) # these two quantile gives us the same things as the box plot.
IQR(x)  # interquantile range (the same as above)
qd(x, c(0,1))
mmm2(x)
```
...
```{r}
# use probs=c(0,1) to set the default (in this case, we do not nee to define the probs for the function to work, but we can also rewrite it.)
...
```{r}
v <- c(1:10, NA) # do not need to know how to handle NA.
qd2(v)
```
...
```{r}
# ... mean we can add in more argument if we want
...
v <- rnorm(100)
qd2(v,type=1)  # we can add our personalised argument into the function if the function contains ...
