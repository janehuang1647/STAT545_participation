...
```r
select(gapminder, continent,everything())## put everything except continent at the back
```
...
```r
## select row of the dataframe
gapminder %>%
  filter(pop>1000000) ##only show lines with population greater than 100million
```
...
## select row of the dataframe
gapminder %>%
  filter(pop>1000000 && continent=="Asia") ##only show lines with population greater than 100million
```
