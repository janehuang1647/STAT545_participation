...
``` r
## join using the variable in the right tibble
...
``` r
## the console will show that the result is by joining x1
left_join(a,b)
```
...
``` r
## since there is no match between x1 and x3
...
``` r
left_join(flights2,airlines) ## console show that it is joined by carriers
```
...
``` r
## using left join and right join might effect the number of rows since they depends on which one to be compare with ## such as x compare with y or y compare with x
```
...
``` r
## intersect (x,z) can not do that they need to have the same column name
```
...
``` r
## in the column binding the column name doesnt matter
```
...
``` r
## since they are the same so return 0
```

### 2. Can we apply `set` and `binding` funcions between `flights2` and `weather` datasets. Why and why not?

``` r
#union(flights2,weather) # this doesnt work since the union need to have same column name
colnames(flights2)
```
...
# bind_cols need to have the same ROW number
# bind_rows doesnt matter
```
...
``` r
p <- tibble(x1=c("A","B","C","B"),X2=1:4)
q <- tibble(x1=c("A","C","D","B"),x3=c(1,3,2,4))

  left_join(p,q) # use p as a reference to find different q
```
...
``` r
  left_join(q,p) # use q as a reference to find different p
```
