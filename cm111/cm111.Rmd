...
```{r}
simple_page %>% 
	html_nodes("p") %>%   # search the p tag
  html_text()   # only showing the content within the p tag
simple_page %>% 
	html_nodes("a") %>% 
  html_text()
simple_page %>% 
...
read_html("https://www.musixmatch.com/search/kane%20brown") %>% 
  html_nodes(".track-card .media-card-title") %>%  # this is used to Selector gadget
  html_text()
